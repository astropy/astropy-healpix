...
void interpolate_weights(double lon, double lat, int64_t *ring_indices,
	...
	if (ring_number == 0) {
		...
		ring_neighbours(lon + dlon, lat_tmp, Nside, &ring3, &ring4, &lon3, &lat3, &lon4, &lat4);
...
void ring_neighbours(double lon, double lat, int Nside,
									   int64_t *ring1, int64_t *ring2,
	...
	if (lon - *lon1 > M_PI) *lon1 += 2 * M_PI;
	if (*lon1 - lon > M_PI) *lon1 -= 2 * M_PI;

	// Also convert to a ring index and decompose into ring number and longitude index
	*ring1 = healpixl_xy_to_ring(xy_index, Nside);
	...
	// Take into account possible wrapping so that the pixel longitude/latitude
	// are close to the requested longitude/latitude
	if (lon - *lon2 > M_PI) *lon2 += 2 * M_PI;
	if (*lon2 - lon > M_PI) *lon2 -= 2 * M_PI;
